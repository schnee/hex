...
### Installation & Local Startup (Backend + Frontend)
...
2.  **Run the backend with `uv` (Terminal 1):**
    ```bash
    cd backend
...
3.  **Set up and run the frontend (Terminal 2):**
    ```bash
    cd frontend
...
4.  **Open the app:**
    Visit `http://localhost:3000` in your browser. The frontend is configured to call the backend at `http://localhost:8000`.

### Legacy Streamlit App (Optional)

If you want to run the original Streamlit UI instead of the React frontend:
...
Production deployment instructions are in `DEPLOYMENT.md`.
