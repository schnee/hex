...
const GENERATED_PNG_DATA_URL_PREFIX = 'data:image/png;base64,';

export const normalizeGeneratedPatternPngData = (pngData: string): string => {
  if (pngData.startsWith('data:image/')) {
    return pngData;
  }

  return `${GENERATED_PNG_DATA_URL_PREFIX}${pngData}`;
};
    ...
    return {
      ...
      data: {
        patterns: response.data.patterns.map(pattern => ({
          ...pattern,
          png_data: normalizeGeneratedPatternPngData(pattern.png_data),
        })),
      },
    };
  }
...
// Export individual wrappers without losing instance context
export const downloadPattern = (patternId: string) =>
  apiClient.downloadPattern(patternId);

export const generatePatterns = (request: GenerateRequest) =>
  apiClient.generatePatterns(request);

export const uploadImage = (file: File, maxDimension?: number) =>
  apiClient.uploadImage(file, maxDimension);

export const calculateOverlay = (request: OverlayRequest) =>
  apiClient.calculateOverlay(request);
